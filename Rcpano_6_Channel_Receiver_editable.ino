...
void setup()
{
  ...
  pinMode(6,OUTPUT);
   ...
   // Start serial communication to print received data
  //Serial.begin(9600);  
...

{
 ...
 // Check if aux1 is pressed (emergency stop condition)
 if (data.aux1 < 100) {  // Threshold for button press (change this value if needed)
    // If aux1 is pressed, set all values to zero to stop the drone
    data.throttle = 0;
    data.pitch = 0;
    data.roll = 0;
   data.yaw = 0;
 ...
 // Serial.print("Throttle: ");
  ...
  //Serial.print("\tAux1: ");
  //Serial.print(data.aux1);  // Print aux1
  //Serial.print("\tAux2: ");
  //Serial.println(data.aux2); // Print aux2


  
  delay(100);  // Delay to reduce Serial Monitor update frequency
}
}
